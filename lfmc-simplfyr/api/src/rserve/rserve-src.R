...
simpfunc <- function(shp, intolerant) {
    ...
    filep <- paste("/home/docker/rserve/data/", shp, sep="", collapse = NULL)
    
    if (!file.exists(filep)) {
        message <- filep
        return(message)
    } else {
        inshp <- shapefile(filep, warnPRJ = F)
        simpshp <- gSimplify(inshp, intolerant, topologyPreserve = F)
        
        if (is.na(crs(simpshp)) == F) {
            newcrs <- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs"
            simpshpll <- spTransform(simpshp, CRS(newcrs))
            # shapefile(simpshpll, paste('simplified.', shp, sep=''), overwrite=TRUE)
            JASON <- geojson_json(simpshpll)
            return(JASON)
        } else {
